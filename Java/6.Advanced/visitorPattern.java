...
class SumInLeavesVisitor extends TreeVis {
  ...
  public void visitNode(TreeNode node) { return; }
...
public class Solution {
  ...
  public static Tree solve() {
    ...
    Tree root =
        new TreeNode((Integer)nodeAtts.get(1)[0], (Color)nodeAtts.get(1)[1], 0);
    tree.put(1, root);
  ...
  private static void DFS(int n, Map<Integer, ArrayList<Integer>> edges,
  ...
  private static void DFSUtil(TreeNode parent, int v, boolean[] visited,
    ...
    if (edges.get(v).size() == 1 && v != 1) {
      TreeLeaf treeLeaf =
          new TreeLeaf((Integer)nodeAtts.get(v)[0], (Color)nodeAtts.get(v)[1],
                       parent.getDepth() + 1);
    ...
    if (v != 1) {
      treeNode = new TreeNode((Integer)nodeAtts.get(v)[0],
                              (Color)nodeAtts.get(v)[1], parent.getDepth() + 1);
